...
```
\ \
In questo modo stiamo comunicando di consentire l'origine differente del frontend (`http://localhost:4200`), consentendo qualsiasi metodo e qualsiasi header.
Questo funziona perchè quando #gls("angular") con #emph("HttpClient") effettua una richiesta verso #gls("fastapi"), il browser esegue prima una *#emph("Preflight Request")* di questo tipo:
\ \
```
...
```
\ \
chiedendo al server se autorizza questa chiamata. Grazie al #emph("Middleware") il nostro server è configurato per accettarla, dunque non riceviamo un messaggio di errore e viene effettuata la richiesta, rispondendo al #emph("Preflight") con una risposta tipo:
\ \
```
HTTP/1.1 200 OK
