...
L'obiettivo dell'integrazione di un Modello Linguistico all'interno di ThreatLens non è delegare un calcolo del rischio classificando la #gls("cve"), ma rielaborare tutti i dati ricavati dai vari provider in una sintesi in linguaggio naturale fornendo un chiaro contesto all'analista di sicurezza.
All'intelligenza artificiale è stata data la direttiva di agire come un analista di #emph("cybersecurity") senior.

Per ottenere risultati affidabili, coerenti e privi di #gls("allucinazioni"), è stata strutturata in input una direttiva che utilizza tecniche di #emph[#gls("prompt-engineering")] con un'infrastruttura di validazione dell'output rigorosa.

== Strutturazione del Contesto e #gls("prompt") Engineering

Il #gls("prompt") inviato all'#gls("llm") non è una semplice stringa testuale, ma un #gls("payload") JSON strutturato che incapsula in modo ordinato tutte le metriche elaborate dalla #gls("pipeline"). A seguito di attente ricerche è emerso che il formato più leggibile da un #gls("llm") è il JSON, perché consente di strutturare il #gls("prompt") in chiavi-valori che aiutano il modello nella comprensione della direttiva e aumentano le performance di risposta in output.
Il #gls("prompt") risiede in un file dedicato che è composto dalle seguenti sezioni:
...
- *ThreatLens Assessment:* Rappresenta il risultato calcolato dal `PriorityEngine`. Al modello viene vietato di ricalcolare, correggere o alterare la priorità calcolata dal sistema, limitandone il ruolo alla sola spiegazione delle motivazioni che hanno portato a tale risultato.
...
Per gestire questa problematica, il sistema calcola a priori una metrica definita #emph("Alignment Observation"), la quale confronta quantitativamente le due valutazioni incasellandole in categorie predefinite (es. valutazioni allineate verso l'alto, oppure rischio dello scanner elevato ma priorità calcolata inferiore). Questa classificazione viene passata al modello AI con regole rigorose: in caso di divergenza, l'IA deve spiegare in linguaggio naturale la differenza osservabile chiarendo che i due sistemi valutano metriche differenti, senza mai presentare tale discordanza come un errore tecnico.
In questo modo verrà posta l'attenzione dell'analista sui motivi di disallineamento delle due metriche fornendo, tramite questo confronto, ulteriore contesto all'analisi.

== Vincoli di Output e Prevenzione delle #gls("allucinazioni")

Affinché la risposta sia utilizzabile dal #gls("frontend"), il sistema impone che l'output finale sia privo di formattazioni esterne (come il Markdown).
