...
- *Prima settiamna*:
  è stato fatto uno studio approfondito delle tecnologie che sono state usate nel progetto, in particolare il framework di angular, con esercizi, tutorial e progetti didattici orientati alle parti del framework che avrei dovuto usare per sviluppare la piattaforma.

- *Seconda settimana*:
  è stato affrontato il dominio del problema: sono stati studiati i concetti di cve e che cosa sia uno scanner. Questo studio è stato integrato dalla lettura di articoli scientifici inerenti alle tematiche del *vulnerability assessment* e della prioritizzazione del rischio.
...
- *Quarta settimana*:
  è continuato lo studio delle tecnologie al fine di comprendere i pattern di organizzazione del codice di angular e FastAPI, in modo da utilizzarle nello sviluppo della piattaforma rispettando la progettazione architetturale.

- *Quinta settimana*:
  è cominciata la codifica del prodotto partendo dalle funzionalità di backend. Si è sviluppato in modo progressivo integrando man mano le parti che contribuissero al sistema di calcolo delle priorità, questa modularità ha facilitato la scrittura dei test e produrre risultati parziali ma concreti durante tutto il ciclo di implementazione.

- *Sesta e settima settimana*:
  è stato creato l'ambiente di test delle macchine virtuali ed è stato integrato lo scanner Qualys nel sistema e l'utilizzo dell'AI per la generazione delle spiegazioni relative alle vulnerabilità.

- *Ottava e nona settimana*:
  sono state dedicate al _*refactoring*_ del codice, rendendo più coerente la _*codebase*_. È stata implementata gestione degli errori personalizzata e granulare, al fine di segnalare in modo preciso e completo all'utente quale parte avesse fallito durante la produzione del report delle vulnerabilità.