...
Durante le prime settimane l'attività si è svolta in modalità ibrida, per poi passare ad un regime completamente da remoto a causa dell'inagibilità della sede aziendale. Sebbene la modalità a distanza sia stata preponderante, l'efficienza operativa e la collaborazione non ne hanno risentito. L'uso di canali di comunicazione sincroni e asincroni come #gls("teams") ha garantito un aggiornamento continuo sull'avanzamento delle attività e un supporto immediato in caso di difficoltà.
...
L'organizzazione delle attività è stata calcolata in divenire, in modo da allocare correttamente le risorse disponibili e avere sotto controllo il raggiungimento degli obiettivi, restando entro il tempo prestabilito dal tirocinio.
In autonomia è stato stilato un calendario settimanale su #gls("notion"), in cui veniva tenuta traccia degli obiettivi giornalieri. 
...
Questo crono-programma veniva poi condiviso a fine settimana per mostrare il progresso raggiunto e condividere le attività che si erano rivelate più onerose, al fine di chiedere un consiglio ai colleghi più esperti per le problematiche ancora aperte.
...
Durante la seconda settimana è continuato lo studio di #gls("angular"), vedendo concetti più avanzati come il #emph("routing"), gli #emph("observer"), la #emph("dependency injection").
In seguito è stato affrontato il dominio del problema, studiando i concetti di #gls("cve") e di #gls("cpe"). Questa analisi è stata integrata dalla lettura di articoli scientifici inerenti alle tematiche del #gls("vulnerability-assessment") e della prioritizzazione del rischio.
...
Durante la quarta settimana è iniziata la progettazione del sistema, definendo le classi di #gls("backend") e le componenti #gls("angular") con le loro principali funzioni. Per formalizzare ed esporre i risultati del lavoro sono stati prodotti dei diagrammi in #gls("uml"), per discutere della direzione progettuale e raffinare il lavoro compiuto.
In aggiunta è stata progettata l'organizzazione della repo e dei suoi file, in modo da poter procedere con uno sviluppo il più possibile ordinato ed efficiente.
...
Per accelerare la codifica di questa parte e testare la validità degli studi fatti riguardo al dominio è stato fatto uso di #emph("fixture") di dati (risposte sintetiche delle chiamate #gls("api") dei vari servizi), in modo da strutturare la logica di #gls("backend") anche senza avere le credenziali di #gls("qualys") o #gls("claude").
...
In seguito è stato completato e stilizzato il frontend, pensato per essere piacevole e funzionale ad una figura come un analista di sicurezza; più precisamente, sono state ultimate:
- pagina di home con lista di scansioni effettuate.
...
Questo approccio serve sia per notificare correttamente l'utente e migliorare la #emph("user experience"), sia per agevolare lo sviluppatore in fase di #emph[#gls("debugging")] della piattaforma.
