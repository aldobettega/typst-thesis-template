...
Durante la prima fase del tirocinio è stato fatto uno studio approfondito sul tema del #gls("vulnerability-assessment"), attraverso il quale sono stati compresi i termini chiave del dominio, le principali problematiche e le soluzioni che le aziende prendono in considerazione per gestire al meglio il tracciamento e la risoluzione di vulnerabilità.
Come riporta Rajamani @rajamani2025, nel solo 2025 sono state pubblicate 48.185 #gls("cve"), di cui il 56% è classificato come #emph("high") o #emph("critical"), rendendo la coda di #gls("remediation") ingestibile senza un #gls("triage") intelligente.
...
Per questo motivo occorre adottare un framework #gls("ctem") (#emph("Continuous Threat Exposure Management")) e affidarsi ad un ampio ventaglio di metriche per poter contestualizzare al meglio le #gls("cve") ed ottimizzare la loro risoluzione, dando priorità ad un ristretto e mirato numero di vulnerabilità.

La piattaforma di ThreatLens si sviluppa in questo contesto e intende ottenere da un indirizzo #gls("ip") una lista di #gls("cve") utilizzando scanner professionali di terze parti come #gls("qualys"). Ai risultati ritornati dallo strumento aggiunge una serie di altre metriche, fornendo un dettagliato e attendibile contesto facilmente interpretabile da un analista di sicurezza.
...
Come evidenziato da Shimizu e Hashimoto @VMC, la metrica #gls("cvss") è stata creata solo per misurare il massimo impatto teorico possibile invece del rischio nel mondo reale. Infatti non considera la probabilità che essa venga usata e il rischio pesato in un preciso contesto.
Già da diverso tempo, numerosi studi in letteratura @ImprovingCvss sottolineano come i punteggi #gls("cvss"), senza informazioni sull'ambiente in cui si presenta la vulnerabilità, abbiano utilità limitata per una prioritizzazione pratica. Inoltre, aggiungere del contesto migliorerebbe significativamente la selezione delle risposte.

Un secondo problema di questa metrica è la scarsa azionabilità del punteggio. Il #gls("cvss") concentra molte vulnerabilità nelle fasce alte, con uno score superiore al 7.0, rendendo difficile stabilire la priorità di #emph("remediation") e aumentando il fenomeno della #gls("vulnerability-fatigue").
...
Un dato significativo è l'#gls("epss"), che utilizza il machine learning per stimare la probabilità che una vulnerabilità venga sfruttata entro 30 giorni, mentre il #gls("kev") indica se sono state registrate evidenze di un #emph[#gls("exploit")] confermato.
...
#figure(
  caption: [Confronto delle metriche di prioritizzazione: Efficienza e Copertura in base al metodo adottato @VMC.],
  kind: table,
  table(
...
+ Nel primo stadio viene controllata la reale minaccia e si passa allo stadio successivo se e solo se una di queste due condizioni è vera:
    - la vulnerabilità è presente nel catalogo del #gls("kev")?
    - presenta un #gls("epss") alto (maggiore di 0.0888)?

+ Se si arriva al secondo stadio, la minaccia è probabile, dunque si valuta quanto sarebbe grave nel peggior caso possibile: se la vulnerabilità presenta un #gls("cvss") maggiore di 7.0 è di priorità massima.

Dal #emph[#gls("decision-tree")] possono essere prodotti in output quattro risultati:
...
Questo framework adotta un approccio #emph("threat-first"): come riportano Shimizu e Hashimoto @VMC, le vulnerabilità realmente sfruttate rispetto alle #gls("cve") pubblicate sono in numero molto minore. Per questo motivo, l'algoritmo parte valutando le vulnerabilità che sono realmente una minaccia. 

=== Parametrizzazione dei valori

Le soglie citate sopra per #gls("cvss") e #gls("epss") sono dei parametri indicati da Shimizu e Hashimoto @VMC che hanno condotto un'analisi usando un dataset di 28.377 #gls("cve"). Lo studio dimostra come questi specifici valori siano statisticamente ottimali per segmentare le minacce in classi operative distinte.

Tuttavia, lo studio indica che questi valori possono essere dei parametri flessibili ed è possibile configurarli in funzione del profilo di tolleranza che si vuole ottenere.
...
In ThreatLens è stato deciso di inserire nell'algoritmo le soglie indicate come statisticamente ottimali da Shimizu e Hashimoto @VMC. Tuttavia, a fronte delle considerazioni sopracitate, è stata presa in considerazione come futuro sviluppo una configurazione dall'interfaccia web del calcolo della priorità. Questa implementazione consentirà all'analista di selezionare queste soglie calibrando l'algoritmo per adattare i risultati allo specifico contesto operativo e alla tolleranza al rischio della propria organizzazione.

=== Ruolo del contesto dell'asset

Il sistema progettato analizza il contesto del dispositivo da analizzare attraverso tre parametri distinti forniti in input dall'utente: l'ambiente di esecuzione, l'esposizione di rete e la criticità sistemica dell'asset. Il sistema non utilizza questi dati per alterare matematicamente la priorità finale della vulnerabilità, ma li elabora per produrre un indicatore parallelo e indipendente che segnala all'analista se il contesto richiede un'attenzione nulla, elevata o forte. Il calcolo è rappresentato dalla matrice @context:
...
Questa scelta progettuale è motivata dallo stato attuale della ricerca scientifica al riguardo. Sebbene la letteratura @agyei2026explainable riconosca che il contesto dell'asset sia un livello informativo cruciale per la prioritizzazione, non esistono ad oggi criteri empiricamente consolidati per convertire tali fattori in pesi numerici esatti.

Per garantire un rigore scientifico ed evitare di generare metriche non validate, si è deciso di trattare il contesto dell'asset come un segnale separato che arricchisce l'analisi generale. In questo modo si evita di inquinare il calcolo della gravità modellato su algoritmi comprovati dalla letteratura del settore @VMC.