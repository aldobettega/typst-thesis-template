...
Codifica il #emph("Vulnerability Management Chaining"), un alvero decisionale (#emph("decision tree")) che prende in input questi tre dati e restituisce una priorità da assegnare alla #gls("cve").
...
+ Nello stadio uno viene controllata la reale minaccia e si passa allo stadio successivo se e solo se uno di questi due valori è vero:
    - è presente nel #gls("kev") catalog?
...
Dal decision tree possono essere prodotti in output 4 risultati:
...
I tre dati da soli forniscono poco contesto, ma combinati si compensano tra di loro, dando un contesto più completo:
...
- l'#gls("epss") ha una forte copertura predittiva, ma proprio per questo è un dato probabilistico con alta incertezza che produce flasi positivi e falsi negativi
...
Se per esempio ci si trova in un ambiente particolarmente critico ed è necessario prendere in considerazione vulnerabilità anche con un valori di gravità minore, si abbassa la soglia #gls("epss")
